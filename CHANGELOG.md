...
* Correct logic in `smc_enable_root_relative_urls`
...
* Return instead of echo class names in `smc_main_class` and `smc_sidebar_class`
* Move nav customizations into `lib/nav.php`

### 6.1.0: October 2nd, 2012
* Change smc_sidebar into a more explicit configuration array
* Re-organize configuration/setup files
* Update to jQuery 1.8.2
* Refactor/simplify SMC vCard Widget
...
* Merge `SMC_Navbar_Nav_Walker` into `SMC_Nav_Walker`
...
* Update `smc.pot` for translations
...
* Remove `smc-options.php` and replaced with a more simple `smc-config.php`
* Now using Bootstrap markup on forms, page titles, image galleries, alerts and errors, post and comment navigation
* Remove SMC styles from `style.css` and introduced `app.css` for site-specific CSS
* Remove almost all previous default SMC styling
...
* Clean up `body_class()` some more with `smc_body_class()`
...
* Add a blank `inc/smc-custom.php` file
...
* Change name from BB to SMC and release to the public
...
* Rewrite static folders in `/wp-content/themes/smc/` (`css/`, `js/`, `img/`) to the root (`/css/`, `/js/`, `/img/`)
